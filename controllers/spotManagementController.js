...
const createSpot = async (req, res) => {
    ...
    if (req.user.type == 'vendor') {
        try {
            const { owner, owner_id, firm_name, address, city, cover_photo, mobile } = req.body;
            if (owner.length > 0 || owner_id.length > 0 || firm_name.length > 0 || address.length > 0
                || city.length > 0 || cover_photo.length > 0) {
                const spot = new TourismSpot({ owner: req.user.fullname, owner_id: req.user._id, 
                    firm_name, address, city, cover_photo, mobile });
                await spot.save()
                res.status(201).json({
                    "result": "Success"
                })
